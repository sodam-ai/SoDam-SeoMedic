...
## 🎉 PRD Phase 1 1순위(마켓 실사용 재검증) — 이 프로젝트 역사상 최초 성공 확인 (2026-09-01)

**배경**: `03_PHASES.md:33`("마켓 플러그인으로 설치 → 다른 프로젝트에서 `/seo-audit` 실행 검증")이
바로 위 여러 라운드에 걸쳐 7~8세션 연속 미해결로 기록돼 있었다("이 프로젝트 역사 전체에서 성공 기록이
단 한 건도 없었다" — 2000번째 줄 참고). 사용자가 이 항목만은 AI 대행이 불가능하다는 걸 받아들여
**완전히 별도의 Claude Code 세션**(다른 볼트 `Dev_Command`)에서 직접 `/seomedic:seo-audit
https://example.com`을 실행하고 그 결과를 공유했다.

### 확인된 사실(사용자가 붙여넣은 실행 결과 원문을 그대로 대조, 추측 아님)
- `seomedic` 스킬 로드 성공 → `plugin:seomedic:seomedic` MCP 서버 연결 성공 → 실제 크롤·CWV 측정·
  규칙 평가 파이프라인이 끝까지 실행되어 리포트 생성됨(에러·타임아웃·연결실패 없음).
- 리포트에 등장한 규칙 ID 4개(`R-CANONICAL-MISSING`·`R-OG-DESCRIPTION-MISSING`·`R-JSONLD-MISSING`·
  `R-QA-STRUCTURE-MISSING`)를 이 저장소의 `rules/definitions/*.ts`에서 직접 grep해 전부 실존 확인 —
  결과가 지어낸 것이 아니라 실제 규칙 엔진 출력이다.
- CWV(LCP 793ms·CLS 0.000·INP≈0ms)·"lab≠field(CrUX) 다를 수 있음" 고지·`example.com`이 IANA 예약
  테스트 도메인이라는 해석까지 전부 이 프로젝트가 설계한 대로의 정상 동작.
- `/seo-audit`는 분석 전용이라 소스 파일 변경 없음 — 리포트 자신도 이를 명시.

### 이게 실제로 무엇을 증명하는지(과대 해석 경계)
- **증명됨**: (1) 마켓플레이스로 설치된 플러그인이 **완전히 새로운 세션**에서 MCP 연결에 성공한다.
  (2) **SeoMedic 소스 저장소가 아닌 다른 프로젝트**(`Dev_Command` 볼트)에서 정상 동작 — PRD가 요구한
  "어느 프로젝트에서나 설치" 요건도 함께 충족. (3) Windows에서 `/seo-audit` 전체 파이프라인이 실제
  라이브 URL로 끝까지 작동. 오늘 새벽 고친 ESM `NODE_PATH` 버그(PR #17)가 실제 정상 설치 경로에서도
  더 이상 재발하지 않음을 뒷받침하는 강한 정황 증거(간접 확인 — 그 버그를 다시 일부러 재현해 A/B 대조한
  것은 아님).
- **아직 증명 안 됨(정직하게 구분)**: Mac/Linux(여전히 AI 접근 불가), **Phase 1.5**(`/seo-fix` —
  실제 Next.js 소스 수정 파이프라인)의 실사용 검증은 이번 건과 별개로 여전히 미확인. 이번 건은
  `/seo-audit`(Phase 1, 읽기 전용) 1건만 검증됐다.

### 결론
`03_PHASES.md:33`의 문구를 문자 그대로 보면 이 항목("`/seo-audit` 실행 검증")은 **Windows 레그가
최초로 충족**됐다. 앞으로의 PRD 재감사 라운드에서 이 항목을 "7~8세션 연속 미해결"로 다시 보고하지
말 것 — 대신 "Windows `/seo-audit` 확인됨 / Mac·Linux 미확인 / Phase 1.5 `/seo-fix` 실사용 미확인"
세 갈래로 정확히 구분해 추적할 것.

### 남은 것
1. Mac/Linux `/seo-audit` 실사용 검증 — 사람 전용, 미확인.
...
## 📋 다음 작업 단계 — 리스크 포함 종합 정리 (2026-09-01, Phase 1+1.5 완료 직후)

**배경**: Phase 1(`/seo-audit`)과 Phase 1.5(`/seo-fix`) 둘 다 실사용 검증이 끝난 시점에서, "다음에
무엇을 해야 하는가"를 보류·추측·누락·오류·빈틈·모순·변수 없이 정리해달라는 요청을 받았다. 이 문서
자신(`HUMAN_ACTION_CHECKLIST.md`)·`CHECKPOINT_1.5.md`·`.PRD/*`를 다시 대조한 결과부터 먼저 밝힌다:
**이 시점 기준으로 AI가 코드로 실행 가능한 작업은 남아있지 않다.** 아래 4건은 전부 사람의 외부
행동(법무 자문·클라우드 콘솔 설정·다른 OS 접근·계정 정리)이 선행 조건이라 구조적으로 AI 대행이
불가능하다 — 이걸 숨기지 않고 그대로 명시한다.

### 발견한 문서 불일치 1건(이번 정리 중 발견, 별도로 이미 수정·병합함)
`HUMAN_ACTION_CHECKLIST.md`의 74번 항목("로컬 `/seo-fix` 실행")이 "대화형 세션에서 직접 실행한 적
없음"이라고 낡게 남아있었는데, 바로 위 섹션에서 기록한 대로 이미 오늘 실제로 실행·검증됐다. PR #25로
정정 완료(이 섹션과 별개 커밋) — 문서 3곳(`CHECKPOINT.md`·`CHECKPOINT_1.5.md`·`HUMAN_ACTION_CHECKLIST.md`)이
서로 다른 상태를 말하고 있으면 다음 세션이 잘못된 판단을 할 위험이 있어, 발견 즉시 닫았다.

### 순위 1 — PSI(PageSpeed Insights) 키 발급·확인 (가장 쉬움, 가장 먼저 권장)
**할 일**: Google Cloud Console에서 API 키 발급 → `PAGESPEED_API_KEY` 설정 → `/seo-audit
https://실사이트.com` 1회 실행.

**미리 고려한 위험**:
- **파싱 가정 미검증**: 코드가 API의 `percentile` 값을 100으로 나눠 CLS(0~1 사이 소수)를 계산하는데,
  이건 공식 문서 예시로만 확인했고 실제 응답으로는 검증된 적이 없다. 실제 값이 예상 범위(0~1)를
  벗어나면(예: 이미 0~1 스케일로 오거나, 필드명이 문서와 다르면) 리포트에 이상한 숫자가 그대로
  노출될 위험이 있다 — 처음 실행 시 CLS 숫자가 상식적인지 반드시 눈으로 확인 필요.
- **키 노출**: `.env.local` 패턴이 이미 `.gitignore`에 있어 커밋 위험은 낮지만, 발급 직후 실수로
  채팅·스크린샷·로그에 키 값 자체를 남기지 않도록 주의.
- **무료 할당량**: PSI API는 무료 할당량이 넉넉해 개인 테스트 수준에서 초과 위험은 낮음.

### 순위 2 — 방치된 GitHub 테스트 토큰·저장소 정리 (낮은 위험이지만 7주째 방치, 빨리 닫을수록 좋음)
**배경**: 2026-07-13 GitHub fork 경로 실증 때 발급한 **Classic PAT(`repo` 스코프 전체)**가 아직
폐기되지 않았고, 테스트용 저장소 2개(`sodam-test/seomedic-fork-test`, 그 fork인
`sodam-ai/seomedic-fork-test`)도 그대로 남아있다(`HUMAN_ACTION_CHECKLIST.md` §2 "후처리 필요").

**미리 고려한 위험**:
- Classic PAT는 fine-grained보다 스코프가 넓다(`repo` 전체 = 모든 저장소 읽기/쓰기). 방치 기간이
  길어질수록 유출 시 피해 범위가 크다 — **네 가지 리스크 항목 중 유일하게 "시간이 갈수록 위험이
  커지는" 항목**이라 우선순위를 낮게 두면 안 된다는 점을 명시한다.
- `gh repo delete`는 과거 CLI 토큰 scope 문제로 실패한 전례가 있어(`HUMAN_ACTION_CHECKLIST.md`:59),
  **웹 화면에서 직접 삭제**해야 한다 — 이번에도 CLI로 시도하다 막히면 그게 재현이지 새 버그가 아니다.
- 정리 순서 권장: ① GitHub 토큰 설정 화면에서 해당 PAT 폐기(Revoke) → ② 두 테스트 저장소를 웹에서
  직접 삭제 → ③ `HUMAN_ACTION_CHECKLIST.md` §2를 "정리 완료"로 갱신.

### 순위 3 — GSC/GA4(검색 성과·방문자 통계) 실연동 확인
**할 일**: 서비스계정 생성 → GSC/GA4 화면에서 그 서비스계정 이메일에 "뷰어"급 권한 부여 →
env var 3개(`GSC_SERVICE_ACCOUNT_PATH`·`GSC_PROPERTY_SCOPE`·`GA4_PROPERTY_ID`) 설정 → 실행.

**미리 고려한 위험**:
- **권한 과다 부여 위험**: 문서가 명시적으로 "뷰어"만 권장한다 — 실수로 "편집자"·"소유자"급 권한을
  주면 서비스계정이 실제 GSC/GA4 설정을 바꿀 수 있는 상태가 된다. 부여 화면에서 권한 등급을 반드시
  재확인할 것.
- **부분 설정 오작동 방지는 이미 코드로 처리됨**: 3개 중 일부만 설정되면 그 기능은 자동으로
  꺼지고 나머지 audit은 정상 진행되도록 이미 설계돼 있다(조용히 잘못된 값이 나가는 상황 자체가
  코드 레벨에서 차단됨) — 이 부분은 이미 안전하니 재검증 불필요.
- **서비스계정 JSON 키 파일**: 절대 저장소에 커밋하지 않는다(`.env*`·JSON 키 경로 모두 gitignore
  대상, 기존 설계 그대로 유지).

### 순위 4 — 법무 검토 6건 + Mac/Linux 실행 검증 (가장 오래 걸림, 병렬 진행 가능)
- **법무 6건**(L1·L2·L4·L5·L12·L13, `HUMAN_ACTION_CHECKLIST.md` §1): 라이선스 카피레프트 최종 확인,
  크롤 콘텐츠 fair use 경계, 상표 지명적 공정사용 경계(Google/Naver/Bing/Next.js/Claude/ChatGPT 등),
  AI 생성물 저작권, 제품명 상표. **위험**: 이 6건이 미확정인 채로 상업적 배포·홍보를 확대하면, 특히
  L5(상표)·L2(카피레프트)가 실제 문제로 불거질 수 있다 — 지금처럼 개인 사용·PUBLIC 저장소 공개 수준
  에서는 급하지 않지만, 배포 범위를 넓히기 **전에** 반드시 선행돼야 한다.
- **Mac/Linux**: Windows에서 이미 확인된 4개 항목(마켓 설치·`/seo-audit`·`better-sqlite3` 네이티브
  모듈·`/seo-fix`)과 별개로, Unix 전용 코드 경로(`npm_execpath` 처리, `HUMAN_ACTION_CHECKLIST.md`
  76번)는 Windows에서 아예 실행되지 않는 분기라 **Windows 검증으로는 대체 불가** — 반드시 실제
  Mac 또는 Linux 환경 필요.

### 결론 및 명시적 경고
- 위 4건 전부 **AI가 코드를 더 작성해서 앞당길 수 있는 종류의 작업이 아니다.** 다음 세션이 이
  섹션을 보고 "코드로 뭔가 더 할 게 없을까"를 찾으려 하면, 그건 실제 필요가 아니라 **불필요한 범위
  확장(scope creep) 위험**이다 — 이미 완료된 기능을 재검증하거나, PRD에 없는 새 기능을 추가하려는
  유혹을 이 문장으로 미리 차단한다.
- Phase 2 성공기준(`03_PHASES.md:98-102`)은 순위 1·3이 끝나야 "코드 완료"를 넘어 "완전 충족"으로
  격상된다 — 이 둘을 **Phase 1.5의 완료 여부와 혼동하지 말 것**(Phase 1.5는 이미 별개로 완전히
  끝났음, 위 섹션 참고). 두 단계를 섞어서 "아직 Phase 1.5도 안 끝났다"는 식으로 잘못 보고하지 않도록
  이 문장으로 경계를 명확히 남긴다.
