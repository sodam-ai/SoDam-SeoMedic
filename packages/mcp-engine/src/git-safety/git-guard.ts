...
export type GitCleanStatus =
  | { clean: true }
  | { clean: false; reason: "dirty" | "not_a_repo" | "git_not_found"; details: string };

/**
 * git argv를 배열로만 spawn한다(셸 문자열 보간 없음 — M2 명령어주입 방지 원칙과 동일).
 * `-- .`로 projectRoot 하위만 스코프해, 모노레포에서 Next 앱 밖의 변경까지 "dirty"로 오탐하지 않는다.
 */
function runGit(projectRoot: string, args: string[]): Promise<{ code: number | null; stdout: string; stderr: string }> {
...
export async function checkGitClean(projectRoot: string): Promise<GitCleanStatus> {
  let result: { code: number | null; stdout: string; stderr: string };
  try {
    result = await runGit(projectRoot, ["status", "--porcelain", "--", "."]);
  } catch (err) {
    return { clean: false, reason: "git_not_found", details: (err as Error).message };
  }
 ...
 */
export function backupFiles(projectRoot: string, targetPaths: string[], runId: string): BackupManifestEntry[] {
  ...
  for (const relPath of targetPaths) {
    ...
    if (!fs.existsSync(absPath)) continue; // 아직 없는 파일(신규 생성 예정)은 백업 대상 아님
...
export async function revertViaGitCheckout(projectRoot: string, targetPaths: string[]): Promise<void> {
