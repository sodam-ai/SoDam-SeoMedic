...
export async function commitAll(repoPath: string, message: string, env?: NodeJS.ProcessEnv): Promise<void> {
  await runGitCommand(["add", "-A"], repoPath, DEFAULT_TIMEOUT_MS);
  await runGitCommand(
    ["-c", "user.email=seomedic-bot@users.noreply.github.com", "-c", "user.name=seomedic-bot", "commit", "-q", "-m", message],
    repoPath,
    DEFAULT_TIMEOUT_MS,
    env,
  );
 ...
 */
export async function pushFixBranch(repoPath: string, branchName: string, env?: NodeJS.ProcessEnv): Promise<void> {
  assertSafeBranchName(branchName);
