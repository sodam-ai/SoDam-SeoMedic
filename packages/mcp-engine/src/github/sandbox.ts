 ...
 */
const RM_RETRY_OPTIONS = { recursive: true, force: true, maxRetries: 5, retryDelay: 300 } as const;
 ...
 */
export function gcOrphanedSandboxes(): void {
 ...
 */
export async function createSandboxClone(opts: CreateSandboxOptions): Promise<SandboxHandle> {
  ...
  try {
    await runGitCommand(
      ["clone", "--depth", "1", opts.cloneUrl, tempDir],
      os.tmpdir(),
