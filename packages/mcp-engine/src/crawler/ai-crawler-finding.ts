...
import { AI_CRAWLER_CATALOG_CONFIRMED_DATE, type AiCrawlerAccessReport, type AiCrawlerPurpose } from "./ai-crawler-policy.js";

export const AI_CRAWLER_POLICY_RULE_ID = "R-AI-CRAWLER-POLICY";
export const AI_CRAWLER_POLICY_RULE_VERSION = 1;

const PURPOSE_LABEL: Record<AiCrawlerPurpose, string> = {
  training: "AI 학습용 수집",
  ai_search: "AI 답변엔진 검색 노출(예: ChatGPT·Claude·Perplexity 검색결과)",
  user_fetch: "사용자가 채팅에서 이 URL을 직접 물어봤을 때의 1회성 접근",
};

const PURPOSE_ORDER: AiCrawlerPurpose[] = ["training", "ai_search", "user_fetch"];

function summarizeGroup(entries: AiCrawlerAccessReport["entries"], purpose: AiCrawlerPurpose): string {
  ...
  const parts: string[] = [];
  if (blocked.length > 0) parts.push(`차단: ${blocked.join(", ")}`);
  if (allowed.length > 0) parts.push(`허용: ${allowed.join(", ")}`);
 ...
 */
export function buildAiCrawlerPolicyViolation(origin: string, report: AiCrawlerAccessReport): RuleViolation {
  ...
  return {
    ...
    currentValue,
    recommendedValue:
      `정보 제공용 — 특정 차단/허용을 권장하지 않습니다. 정책 채택은 사이트 운영자 결정 사항입니다. ` +
