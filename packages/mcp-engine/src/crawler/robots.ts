...
import { evaluateAiCrawlerAccess, type AiCrawlerAccessReport } from "./ai-crawler-policy.js";
...
type RobotsFetchStatus = "found" | "absent" | "unavailable";
 ...
 */
async function fetchRobotsTxtRaw(origin: string): Promise<RobotsFetchResult> {
  ...
  if (statusCode === 404) return { status: "absent", robotsUrl, body: "" };
  if (statusCode !== 200) return { status: "unavailable", robotsUrl, body: "" };
...
export async function loadRobotsPolicy(origin: string): Promise<RobotsPolicy> {
  ...
  if (fetched.status === "absent") return allowAllPolicy();
  if (fetched.status === "unavailable") return denyAllPolicy();
 ...
 */
export async function loadAiCrawlerAccess(origin: string): Promise<AiCrawlerAccessReport | null> {
  const fetched = await fetchRobotsTxtRaw(origin);

  if (fetched.status === "unavailable") return null;
