 ...
 */
export async function fetchLocalBridgeHtml(rawUrl: string, expectedOrigin: string, timeoutMs = DEFAULT_TIMEOUT_MS): Promise<LocalFetchResult> {
  ...
  try {
    ...
    const html = await readBodyWithLimit(res as any, MAX_RESPONSE_BYTES);
