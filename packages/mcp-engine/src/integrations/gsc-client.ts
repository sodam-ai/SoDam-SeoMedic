...
import type { GscClient, GscSearchAnalyticsSummary } from "./types.js";
...
export type GscFetcher = (url: string, init: { headers: Record<string, string>; body: string }) => Promise<GscFetcherResult>;

async function defaultGscFetcher(url: string, init: { headers: Record<string, string>; body: string }): Promise<GscFetcherResult> {
...
export function createGscClient(
  keyFilePath: string,
  fetcher: GscFetcher = defaultGscFetcher,
  ...
  return {
    async fetchSearchAnalyticsSummary(propertyScope: string): Promise<GscSearchAnalyticsSummary> {
      ...
      try {
        ...
        throw new GscApiError(`GSC 인증 실패: ${(err as Error).message}`);
      }
      ...
      let res: GscFetcherResult;
      try {
        res = await fetcher(url, { headers: { "content-type": "application/json", authorization: `Bearer ${token}` }, body });
      } catch (err) {
        throw new GscApiError(maskToken(`GSC API 호출 실패: ${(err as Error).message}`, token));
      }
