 ...
 */
export function findMissingSitemapPaths(crawledUrls: string[], sitemapUrls: string[]): MissingSitemapResult {
  const sitemapPaths = new Set(
    sitemapUrls.map(normalizedPathname).filter((p): p is string => p !== null),
  );
  const baseOriginInferred = sitemapUrls.length > 0 ? (new URL(sitemapUrls[0]).origin ?? null) : null;

  const crawledPaths = Array.from(
    new Set(crawledUrls.map(normalizedPathname).filter((p): p is string => p !== null)),
  );
