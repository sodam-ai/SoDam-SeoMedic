 ...
 */
function tryInsertNewMetadataExport(filePath: string, h1Title: string | null): TitleFixPlan | null {
  if (!fs.existsSync(filePath)) return null;
  ...
  const generateMetadataFn = sourceFile.getFunctions().find((fn) => fn.isExported() && fn.getName() === "generateMetadata");
  if (generateMetadataFn) return null; // 기존 경로가 처리(제네릭 report_only 메시지)

  const metadataDecl = sourceFile.getVariableDeclaration("metadata");
  if (metadataDecl) return null; // metadata가 이미 어떤 형태로든 있음 — 기존 경로가 처리
 ...
 */
export function planTitleFix(filePath: string, h1Title: string | null): TitleFixPlan {
...
export function writeTitleFix(filePath: string, updatedText: string): void {
