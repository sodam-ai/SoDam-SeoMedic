...
function findStaticMetadataObject(filePath: string): { project: Project; objLit: ObjectLiteralExpression } | null {
  ...
  if (generateMetadataFn) return null; // 동적 함수 — 정적 분석 대상 아님

  const metadataDecl = sourceFile.getVariableDeclaration("metadata");
  if (!metadataDecl) return null; // metadata export 자체가 없음 — 새로 만들지 않고 report_only(추측 금지)

  const varStatement = metadataDecl.getVariableStatement();
  if (!varStatement?.isExported()) return null; // export 안 된 동명 변수는 Next.js가 인식하지 않음

  const objLit = metadataDecl.getInitializerIfKind(SyntaxKind.ObjectLiteralExpression);
  if (!objLit) return null; // 변수 참조·함수 호출 등 — 정적 분석 불가

  if (hasSpreadElement(objLit)) return null; // 스프레드가 섞이면 실제 필드 구성을 정적으로 확신할 수 없음
 ...
 * 이 fixer의 범위 밖).
 */
export function planCanonicalFix(filePath: string, canonicalPath: string): CanonicalFixPlan {
...
export function writeCanonicalFix(filePath: string, updatedText: string): void {
