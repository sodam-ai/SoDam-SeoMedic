import { SyntaxKind, type ObjectLiteralExpression, type ArrayLiteralExpression, type Expression, type ObjectLiteralElementLike } from "ts-morph";
 ...
 * (추측 금지, fail-closed).
 */
function getStaticPropertyKey(prop: ObjectLiteralElementLike): string | null {
  const nameNode =
    prop.asKind(SyntaxKind.PropertyAssignment)?.getNameNode() ??
  ...
  if (!nameNode) return null; // SpreadAssignment 등 이름 자체가 없는 요소 — 판별 불가로 처리
  ...
  if (numericLiteral) return numericLiteral.getText();
...
export function assertFieldAbsent(objLiteral: ObjectLiteralExpression, key: string): void {
  for (const prop of objLiteral.getProperties()) {
    ...
    if (name === null) {
      throw new AddSafeViolationError(
        `필드 "${key}" 존재 여부를 정적으로 확신할 수 없는 프로퍼티가 있음(계산된 키 등) — 추측 금지, 존재로 간주`,
      );
    }
    if (name === key) {
