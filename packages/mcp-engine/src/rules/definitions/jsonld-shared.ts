 ...
 */
export function parseJsonLdNodes(text: string): JsonLdNode[] | null {
  let parsed: unknown;
  try {
  ...
  for (const item of topItems) {
    ...
    if (Array.isArray(obj["@graph"])) nodes.push(...(obj["@graph"] as JsonLdNode[]));
    else nodes.push(obj);
  }
  return nodes;
}
...
export function getTypes(node: JsonLdNode): string[] {
  const t = node["@type"];
