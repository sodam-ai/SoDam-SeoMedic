...
type BlockStatus = "valid" | "malformed_json" | "not_object" | "missing_context_or_type";
 ...
 */
function classifyBlock(text: string): BlockStatus {
  let parsed: unknown;
  try {
  ...
  const reachableTypeNodes: unknown[] = [];
  let hasAnyContext = false;

  for (const item of topItems) {
    if (typeof item !== "object" || item === null) return "not_object";
    const obj = item as Record<string, unknown>;
    if ("@context" in obj) hasAnyContext = true;
    if (Array.isArray(obj["@graph"])) reachableTypeNodes.push(...(obj["@graph"] as unknown[]));
    else reachableTypeNodes.push(obj);
  }

  const hasAnyType = reachableTypeNodes.some(
    (node) => typeof node === "object" && node !== null && "@type" in (node as Record<string, unknown>),
  );

  if (!hasAnyContext || !hasAnyType) return "missing_context_or_type";
  return "valid";
}
 ...
 */
export const jsonLdMissingRule: Rule = {
  id: RULE_ID_MISSING,
  version: VERSION,
  category: "schema",
  evaluate(ctx): RuleViolation | null {
    if (ctx.statusCode !== 200) return null;
    if (ctx.renderedSignals.jsonLdBlocks.length > 0) return null;
    return {
 ...
 */
export const jsonLdInvalidRule: Rule = {
  id: RULE_ID_INVALID,
  version: VERSION,
  category: "schema",
  evaluate(ctx): RuleViolation | null {
    ...
    for (let i = 0; i < blocks.length; i++) {
      ...
      const reason: Record<Exclude<BlockStatus, "valid">, string> = {
      ...
      return {
        ...
        severity: "high",
        pageUrl: ctx.pageUrl,
        currentValue: `블록 ${i + 1}: ${reason[status as Exclude<BlockStatus, "valid">]}`,
