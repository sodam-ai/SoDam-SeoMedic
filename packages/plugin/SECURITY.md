...
- 승인 없이 canonical/noindex/robots/sitemap/JSON-LD/title/meta/alt 변경(Phase 1은 애초에 소스 수정 자체를 하지 않음)
...
**현재 상태(2026-07-03 실측)**: high/critical 취약점 0건. moderate 17건은 전부 `lighthouse` 패키지가 번들한 계측(@opentelemetry/@sentry) 라이브러리의 전이 의존성이며, 이 프로젝트는 해당 계측 코드 경로(amqplib·mongoose·hapi 등)를 호출하지 않습니다. 최신 상태는 `npm audit`으로 직접 재확인할 수 있습니다.
