...
async function defaultGa4Fetcher(url, init) {
 ...
 */
export function createGa4Client(keyFilePath, fetcher = defaultGa4Fetcher, tokenProvider = defaultAccessTokenProvider) {
    return {
        async fetchKeyMetrics(propertyId) {
            ...
            try {
                ...
                throw new Ga4ApiError(`GA4 인증 실패: ${err.message}`);
            }
            const end = new Date();
            ...
            try {
                ...
                throw new Ga4ApiError(maskToken(`GA4 API 호출 실패: ${err.message}`, token));
            }
            if (res.status !== 200) {
