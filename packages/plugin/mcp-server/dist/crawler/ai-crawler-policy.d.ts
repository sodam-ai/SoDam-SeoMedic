export type AiCrawlerPurpose = "training" | "ai_search" | "user_fetch";
export interface AiCrawlerCatalogEntry {
    /** robots.txt의 User-agent 토큰(대소문자 무관, robots-parser가 자체 정규화) */
    token: string;
    vendor: string;
 ...
 */
export declare const AI_CRAWLER_CATALOG: AiCrawlerCatalogEntry[];
export declare const AI_CRAWLER_CATALOG_CONFIRMED_DATE = "2026-07-14";
export interface AiCrawlerAccessEntry extends AiCrawlerCatalogEntry {
    allowed: boolean;
}
export interface AiCrawlerAccessReport {
    /** false면 robots.txt 자체가 없어 전체 허용(사양 기본값)이 확정된 상태 */
    robotsTxtFound: boolean;
    entries: AiCrawlerAccessEntry[];
}
/** robots.ts의 ParsedRobots와 구조적으로 호환되는 최소 인터페이스(이 파일이 robots.ts를 직접 import하지 않도록) */
export interface MinimalRobotsQuery {
 ...
 */
export declare function evaluateAiCrawlerAccess(robot: MinimalRobotsQuery | null, robotsTxtStatus: "found" | "absent", rootUrl: string): AiCrawlerAccessReport;
