 ...
 */
export function detectNextjs(projectRoot) {
...
function readInstalledNextVersion(projectRoot) {
...
function detectRouter(projectRoot) {
    ...
    if (appRouterMarkers.some((p) => fs.existsSync(path.join(projectRoot, p))))
        return "app";
    if (pagesRouterMarkers.some((p) => fs.existsSync(path.join(projectRoot, p))))
        return "pages";
    return "unknown";
}
//# sourceMappingURL=detect-nextjs.js.map