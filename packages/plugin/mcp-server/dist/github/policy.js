 ...
 */
export function evaluateRepoPolicy(input) {
    if (input.isArchived) {
    ...
    const warnings = [];
    if (!input.hasLicense)
        warnings.push("LICENSE 파일이 없습니다 — 저장소의 기여 규칙을 알 수 없어 주의가 필요합니다");
    if (!input.hasContributing)
        warnings.push("CONTRIBUTING 가이드가 없습니다 — PR 제출 관례를 미리 확인할 수 없습니다");
