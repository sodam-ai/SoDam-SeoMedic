 ...
 */
export function buildFixBranchName(ruleId) {
    const slug = ruleId
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, "-")
        .replace(/^-+|-+$/g, "");
 ...
 */
export function buildPrContent(fixes) {
    ...
    for (const { fix, finding } of fixes) {
        lines.push(`- **${finding.rule_id}**(${fix.risk_level}) — ${finding.page_url}`, `  - 현재: ${finding.current_value ?? "없음"}`, `  - 권장: ${finding.recommended_value ?? "-"}`, `  - 대상 파일: \`${fix.target_path ?? "-"}\``);
    }
    lines.push("", "## 안전 정보", "- 이 PR은 SeoMedic이 자동으로 생성했습니다(SeoMedic Claude Code 플러그인).", "- 기본 브랜치에 직접 push하지 않았고, 자동 머지도 하지 않았습니다 — 병합 여부는 저장소 관리자가 결정합니다.", "- 적용 전 `next build`로 재검증했습니다.");
