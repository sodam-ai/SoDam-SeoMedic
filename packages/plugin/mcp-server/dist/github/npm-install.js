...
function resolveNpmCliJs() {
    ...
    if (!found) {
        throw new NpmInstallError(`npm CLI 진입점을 찾을 수 없습니다(확인한 경로: ${candidates.join(", ")}) — Node.js 설치를 확인해주세요`);
    }
    return found;
}
/**
 ...
 */
export function installDependenciesSafely(projectRoot, timeoutMs = DEFAULT_TIMEOUT_MS) {
    const lockfile = detectLockfile(projectRoot);
    if (lockfile === "yarn.lock" || lockfile === "pnpm-lock.yaml") {
        return Promise.reject(new NpmInstallError(`이 저장소는 ${lockfile}을 사용합니다 — 현재 GitHub 자동 수정은 npm 프로젝트만 지원합니다`));
    }
    ...
    return new Promise((resolve, reject) => {
        ...
        child.on("close", (code) => {
            ...
            if (code === 0)
                ...
                reject(new NpmInstallError(`의존성 설치 실패(exit ${code}): ${stderr.slice(-2000)}`));
        });
    });
}
