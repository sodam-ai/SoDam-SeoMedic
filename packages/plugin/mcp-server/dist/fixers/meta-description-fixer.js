 ...
 * 폴백한다.
 */
function tryInsertNewMetadataExport(filePath, description) {
    if (!fs.existsSync(filePath))
        return null;
    const project = new Project({ useInMemoryFileSystem: false, skipAddingFilesFromTsConfig: true });
    const sourceFile = project.addSourceFileAtPath(filePath);
 ...
 */
export function planMetaDescriptionFix(filePath, mainFirstParagraphText) {
...
export function writeMetaDescriptionFix(filePath, updatedText) {
    fs.writeFileSync(filePath, updatedText, "utf-8");
}
//# sourceMappingURL=meta-description-fixer.js.map