 ...
 */
function planNoindexFixForFinding(db, projectRoot, finding) {
    const pathname = new URL(finding.page_url).pathname;
    const pageRelPath = findPageFilePath(projectRoot, pathname);
    if (!pageRelPath)
        return null; // 정적 페이지 파일을 못 찾음(동적 라우트 등) — report_only로 남김
...
function resolveRootLayoutPath(projectRoot) {
 ...
 */
function planJsonLdWebsiteFixForFinding(db, projectRoot, finding, pages) {
    ...
    if (!siteName)
        return null; // 복사할 값이 없음 — report_only(값 발명 금지, og-fixer.ts와 동일 원칙)
    const absPath = path.join(projectRoot, targetPath);
 ...
 */
function planTitleFixForFinding(db, projectRoot, finding, pages) {
    const page = pages.find((p) => p.logicalUrl === finding.page_url);
    if (!page)
        return null; // 방어적 fail-closed(있어선 안 되는 상황)
    const h1Title = page.renderedH1Text;
    if (!h1Title)
        return null; // 복사할 값이 없음 — report_only로 남김
    const pathname = new URL(finding.page_url).pathname;
    const pageRelPath = findPageFilePath(projectRoot, pathname);
    if (!pageRelPath)
        return null; // 정적 페이지 파일을 못 찾음(동적 라우트 등) — report_only로 남김
