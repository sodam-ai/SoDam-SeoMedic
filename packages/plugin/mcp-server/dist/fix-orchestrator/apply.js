...
import { findApplicableFixesByAuditRun, markApplied } from "../db/repositories/fix.js";
...
export async function applyLocalFixes(db, projectRoot, auditRunId, options = {}) {
    ...
    if (!gitStatus.clean) {
        throw new FixApplyBlockedError(gitStatus.reason, `git 상태가 clean이 아니라 적용할 수 없습니다(${gitStatus.reason}): ${gitStatus.details}`);
    }
    const outcomes = [];
 ...
 */
async function applyOneFix(db, projectRoot, fix) {
    if (!fix.target_path) {
...
async function applySitemapFix(db, projectRoot, fix) {
    const absPath = path.join(projectRoot, fix.target_path);
    const missingUrls = JSON.parse(fix.idempotency_marker ?? "[]");
...
async function applyCanonicalFix(db, projectRoot, fix) {
    const absPath = path.join(projectRoot, fix.target_path);
...
async function applyOgFix(db, projectRoot, fix) {
    const absPath = path.join(projectRoot, fix.target_path);
    const marker = JSON.parse(fix.idempotency_marker ?? "{}");
    // apply 직전 재검증(TOCTOU + 멱등성) — plan 이후 파일 구조가 바뀌었을 수 있다.
 ...
 * idempotency_marker에 siteName 문자열을 그대로 담아 재검증 시 그대로 재사용한다. */
async function applyJsonLdWebsiteFix(db, projectRoot, fix) {
    const absPath = path.join(projectRoot, fix.target_path);
    const siteName = fix.idempotency_marker ?? "";
    // apply 직전 재검증(TOCTOU + 멱등성) — plan 이후 파일 구조가 바뀌었을 수 있다.
    const recheck = planJsonLdWebsiteFix(absPath, siteName);
    if (!recheck.applicable) {
        return { fixId: fix.id, targetPath: fix.target_path, outcome: "structure_changed", detail: recheck.reason };
    }
    if (recheck.updatedText === recheck.originalText) {
    ...
    const backupManifest = backupFiles(projectRoot, [fix.target_path], `fix-${fix.id}`);
    const backupPath = backupManifest[0]?.backupPath ?? null;
    writeJsonLdWebsiteFix(absPath, recheck.updatedText);
    try {
        await runNextBuildOnly(projectRoot);
    }
    catch (err) {
...
async function applyMetaDescriptionFix(db, projectRoot, fix) {
    const absPath = path.join(projectRoot, fix.target_path);
