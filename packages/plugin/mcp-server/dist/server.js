...
server.registerTool("seomedic_fix_github", {
    ...
    description: "⚠️ 실제 GitHub API로 아직 검증되지 않은 기능입니다(타입체크만 통과). GitHub 저장소를 대상으로 add_safe 수정을 적용하고 PR을 생성합니다(제안, main 직접 push·자동 머지 없음). SEOMEDIC_GITHUB_TOKEN 환경변수가 필요합니다. 색인·표시에 영향을 주는(gated) 항목은 이 1회성 흐름에서 대화형 승인이 불가능해 자동 적용하지 않고 보고만 합니다.",
    inputSchema: {
