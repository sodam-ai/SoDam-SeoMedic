...
server.registerTool("seomedic_fix_github", {
    ...
    description: "GitHub 저장소를 대상으로 add_safe 수정을 적용하고 PR을 생성합니다(제안, main 직접 push·자동 머지 없음). 본인 소유 저장소는 실제 GitHub에서 PR 생성까지 검증 완료(2026-07-04). fork(남의 저장소) 경로는 fork 생성·복제까지 실제 GitHub로 검증됐으나 ⚠️ 마지막 PR 생성 단계는 아직 미검증입니다. SEOMEDIC_GITHUB_TOKEN 환경변수가 필요합니다. 색인·표시에 영향을 주는(gated) 항목은 이 1회성 흐름에서 대화형 승인이 불가능해 자동 적용하지 않고 보고만 합니다.",
    inputSchema: {
    ...
    try {
        ...
        if (result.gatedFindings.length > 0) {
            lines.push("", `### 승인이 필요해 자동 적용하지 않은 항목(${result.gatedFindings.length}건)`, ...result.gatedFindings.map((f) => `- ${f.rule_id} · ${f.page_url}`));
        }
        if (result.reportOnlyFindings.length > 0) {
            lines.push("", `### 자동 수정 불가(제안만, ${result.reportOnlyFindings.length}건)`);
        }
