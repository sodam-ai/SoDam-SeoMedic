...
function renderGithubFixBucket(label, bucket) {
    const lines = [`### ${label} — ${bucket.branchName}`];
    if (bucket.duplicateSkipped) {
        lines.push("이미 같은 브랜치로 열린 PR이 있어 건너뛰었습니다(중복 방지).");
        return lines;
    }
    if (bucket.pr) {
        lines.push(`- PR: ${bucket.pr.url}`, "⚠️ main 브랜치에 직접 반영되지 않았고 자동 머지되지도 않았습니다 — 저장소 관리자가 diff를 검토한 뒤 직접 병합해야 합니다.");
        return lines;
    }
