...
import { createBaseline, findLatestBaseline } from "./db/repositories/baseline.js";
...
server.registerTool("seomedic_audit", {
    title: "SEO/GEO 진단",
    description: "URL(또는 사이트)을 크롤+렌더링해 raw/rendered 신호 차이·CWV·규칙 위반을 진단하고 Markdown 리포트를 반환합니다. 분석 전용(소스 미수정).",
    ...
    try {
        ...
        if (result.skippedByRobots.length > 0)
            notes.push(`robots.txt 정책 또는 사이트 접속 실패로 건너뛴 URL ${result.skippedByRobots.length}개 — URL이 정확한지, 사이트가 접속 가능한지 확인해주세요`);
        if (result.truncated)
            notes.push("max-pages 상한에 도달해 크롤이 잘렸습니다");
        const footer = notes.length > 0 ? `\n\n> 참고: ${notes.join(" · ")}` : "";
        return { content: [{ type: "text", text: markdown + footer }] };
    }
    finally {
        result.db.close();
    }
});
server.registerTool("seomedic_save_baseline", {
    title: "베이스라인 저장",
    description: "가장 최근에 실행한 seomedic_audit 결과를 회귀 비교 기준(베이스라인)으로 저장합니다. 새로 진단을 다시 돌리지 않고, 이미 있는 최근 감사 결과를 그대로 기준점으로 삼습니다. 사용자가 명시적으로 요청했을 때만 호출하세요(자동 저장 금지).",
    ...
    try {
        ...
        if (!latestRun) {
            return {
                content: [
                    { type: "text", text: "먼저 `seomedic_audit`을 실행해 진단 결과를 만든 뒤 베이스라인을 저장해주세요." },
                ],
            };
        }
...
server.registerTool("seomedic_check", {
    title: "회귀(원복) 확인",
    ...
    try {
        ...
        if (revertedKeys.length === 0) {
            return { content: [{ type: "text", text: "베이스라인 대비 원복된 항목이 없습니다." }] };
        }
        ...
        if (regressionKeys.length > 0) {
            ...
            for (const key of regressionKeys) {
                ...
                if (f)
                    lines.push(`- \`${f.rule_id}\` — ${f.page_url} (${f.severity})`);
            }
        }
...
server.registerTool("seomedic_fix_plan", {
    title: "수정 계획 생성(dry-run)",
    description: "로컬 Next.js 프로젝트를 대상으로 로컬 서버를 기동해 진단하고, '없는 것 추가'(add_safe)는 자동 계획, 색인·표시에 영향을 주는 변경(gated)은 승인 대기로 계획만 만듭니다(파일 미수정, dry-run). git 상태가 clean이 아니면 거부합니다.",
    inputSchema: FIX_PROJECT_ROOT_INPUT,
}, async ({ projectRoot }) => {
    const db = openSeomedicDb(projectRoot);
    try {
        const result = await planLocalFix(db, projectRoot);
        const lines = [`## 수정 계획 (audit_run_id=${result.auditRunId})`];
        if (result.truncated)
            lines.push("> 참고: max-pages 상한에 도달해 크롤이 잘렸습니다.");
        ...
        if (result.reportOnlyFindings.length > 0) {
            ...
            for (const f of result.reportOnlyFindings)
                lines.push(`- ${f.rule_id} · ${f.page_url} (${f.severity})`);
        }
        if (result.plannedFixes.length === 0 && result.reportOnlyFindings.length === 0) {
            ...
            lines.push("", `적용하려면 \`seomedic_fix_apply\`(auditRunId=${result.auditRunId})를 호출하세요(auto이거나 승인된 것만 실제로 파일에 반영됩니다). add_safe 수정도 실제 파일 변경이니 커밋·배포 전에 diff를 검토하세요.`);
        }
        return { content: [{ type: "text", text: lines.join("\n") }] };
    }
    catch (err) {
        if (err instanceof FixPlanBlockedError) {
            return { content: [{ type: "text", text: `수정 계획을 만들 수 없습니다: ${err.message}` }] };
        }
        throw err;
    }
    finally {
...
server.registerTool("seomedic_fix_approve", {
    title: "gated 수정 승인",
    ...
    try {
        ...
        if (!result.changed) {
            return {
                content: [
                    { type: "text", text: `fix id=${fixId}는 이미 처리되었거나 존재하지 않습니다(현재 상태: ${result.fix?.approval_status ?? "없음"}).` },
                ],
            };
        }
        return { content: [{ type: "text", text: `fix id=${fixId} 승인 완료. \`seomedic_fix_apply\`로 적용하세요.` }] };
    }
    finally {
        db.close();
    }
});
server.registerTool("seomedic_fix_reject", {
    title: "gated 수정 거부",
    ...
    try {
        ...
        return { content: [{ type: "text", text: `fix id=${fixId} 거부됨(적용되지 않습니다).` }] };
    }
    finally {
        db.close();
    }
});
server.registerTool("seomedic_fix_apply", {
    title: "수정 적용",
    description: "seomedic_fix_plan에서 auto(add_safe) 또는 승인된 fix를 실제 파일에 반영합니다. 적용 후 `next build`로 재검증하고, 실패하면 그 fix만 자동으로 롤백합니다. git 상태가 clean이 아니면 거부합니다.",
    ...
    try {
        ...
        if (outcomes.length === 0) {
            return {
                content: [
                    { type: "text", text: "적용할 fix가 없습니다(전부 이미 적용됐거나 gated 승인 대기 중일 수 있습니다)." },
                ],
            };
        }
        const lines = ["## 적용 결과"];
        for (const o of outcomes)
            lines.push(`- fix id=${o.fixId} (${o.targetPath ?? "-"}): ${o.outcome} — ${o.detail}`);
        ...
        if (err instanceof FixApplyBlockedError) {
            return { content: [{ type: "text", text: `적용할 수 없습니다: ${err.message}` }] };
        }
        throw err;
    }
    finally {
...
server.registerTool("seomedic_fix_rollback", {
    title: "적용된 수정 되돌리기",
    ...
    try {
        ...
        return { content: [{ type: "text", text: `fix id=${fixId} (${result.targetPath})을 되돌렸습니다.` }] };
    }
    catch (err) {
        if (err instanceof FixRollbackError) {
            return { content: [{ type: "text", text: `되돌릴 수 없습니다: ${err.message}` }] };
        }
        throw err;
    }
    finally {
...
server.registerTool("seomedic_fix_github", {
    title: "GitHub 저장소 자동 수정(PR 제안) — 부분 검증",
    description: "GitHub 저장소를 대상으로 수정 사항을 브랜치에 반영해 PR을 생성합니다(제안, main 직접 push·자동 머지 없음 — PR 자체가 승인 절차이며, 병합 여부는 저장소 관리자가 직접 결정합니다). 본인 소유 저장소는 실제 GitHub에서 PR 생성까지 검증 완료(2026-07-04). fork(남의 저장소) 경로는 fork 생성·복제까지 실제 GitHub로 검증됐으나 ⚠️ 마지막 PR 생성 단계는 아직 미검증입니다. SEOMEDIC_GITHUB_TOKEN 환경변수가 필요합니다. 색인·표시에 영향을 주는(gated) 항목(canonical·OG·noindex·robots.ts 등)도 PR diff에 함께 포함되니, 병합 전 반드시 diff를 직접 검토하세요.",
    inputSchema: {
    ...
    try {
        ...
        const lines = [`## GitHub 수정 결과 — ${ref.owner}/${ref.repo}`];
        ...
        if (result.duplicateSkipped) {
            ...
            lines.push("", "### PR 생성됨", `- ${result.pr.url}`, "", "⚠️ main 브랜치에 직접 반영되지 않았고 자동 머지되지도 않았습니다 — 저장소 관리자가 diff를 검토한 뒤 직접 병합해야 합니다.");
        }
        ...
        if (result.gatedFixes.length > 0) {
            ...
            const appliedIds = new Set(result.applied.filter((a) => a.outcome === "applied" || a.outcome === "already_applied").map((a) => a.fixId));
            ...
            if (includedInPr.length > 0) {
                lines.push("", `### PR에 포함된 승인 필요 항목(${includedInPr.length}건) — 병합 전 diff를 직접 검토해주세요`, ...includedInPr.map((f) => `- ${f.finding.rule_id} · ${f.finding.page_url}`));
            }
            if (notIncluded.length > 0) {
                lines.push("", `### 적용하지 못한 항목(${notIncluded.length}건)`, ...notIncluded.map((f) => `- ${f.finding.rule_id} · ${f.finding.page_url}`));
            }
        }
        if (result.reportOnlyFindings.length > 0) {
            lines.push("", `### 자동 수정 불가(제안만, ${result.reportOnlyFindings.length}건)`);
        ...
        if (err instanceof InvalidRepoUrlError || err instanceof GithubTokenError || err instanceof GithubFixBlockedError) {
            return { content: [{ type: "text", text: `GitHub 수정을 진행할 수 없습니다: ${err.message}` }] };
        }
        throw err;
    }
});
