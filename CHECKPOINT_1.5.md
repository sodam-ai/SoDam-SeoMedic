...
**추가 완료(2026-07-06, CI 신설과 같은 세션)**: README/GUIDE/FAQ(한/영×md/html, 8개 파일) 최신화 —
"업데이트 내용 요약"을 `<details>` 토글로 전환 + 위 CI 결과 반영, LICENSE 저작권자 임시확정 상태를
정확히 반영(과장·축소 없이). GitHub About/Topics는 이미 정확해 변경 불필요.

**브랜치 정책 확인 완료(2026-07-06)**: 사용자가 이 세션 중 "main/master는 직접 push 금지, 기능
브랜치+PR로"라는 일반 규칙을 제시했으나, 실제 충돌 상황(이 문서에 추가 기록을 커밋하는 시점)에서
사용자에게 직접 확인한 결과 **"기존대로 master 직접 push 유지"로 확정**됐다(커밋 `a4a2efa`).
이 저장소는 1인 개발·브랜치 전략 없음이 재확인된 것 — ~~**앞으로 이 질문을 다시 묻지 말 것.**~~

> ⚠️ **2026-08-09 정정(`git log` 실측, 이 지시를 그대로 따르면 위험)**: 위 결정은 **2026-07-15까지만
> 실제로 지켜졌다**(`ed7eba5`가 master에 대한 마지막 non-merge 직접 커밋). **2026-07-26 PR #1부터
> 지금까지(PR #1~#10, 한 달 넘게, 이번 세션의 PR #8·#9·#10 포함) 저장소는 정확히 정반대로 —
> 기능 브랜치 + PR 리뷰 방식으로만 — 운영되고 있다.** `.PRD/04_PROJECT_SPEC.md`의 "절대 하지 마"
> 항목도 "main 직접 push·force-push·자동 머지 금지"를 명시한다. **"앞으로 이 질문을 다시 묻지
> 말 것"이라는 위 지시를 지금 문자 그대로 따르면 한 달간 확립된 실제 관행과 정면으로 충돌한다.**
> 다음 세션(또는 지금 세션)은 이 절만 보고 "master 직접 push가 여전히 유효한 정책"이라 단정하지
> 말 것 — 원문은 2026-07-06 시점의 결정 이력으로 보존하되, **2026-07-26 이후 실제 관행(브랜치+PR)이
> 우선한다.** 언제·왜 정책이 다시 바뀌었는지는 이 문서·`CHECKPOINT.md` 어디에도 명시적 기록이 없어
> 단정하지 않는다(추측 금지) — 다만 관찰된 사실(1개월+ 일관된 PR 기반 운영)만 기록한다.
