...
The safety mechanisms have been verified through real execution — there is no direct-push-to-default-branch, force-push, or auto-merge capability in the code at all, and anything requiring approval is never auto-applied. The "your own repository" path (creating a branch and PR directly) has been confirmed working against real GitHub, all the way through PR creation. **The "fork a repository you don't own, then propose via PR" path has been verified through fork creation and cloning against real GitHub, but the final step of actually creating the Pull Request has not been confirmed yet** (the test repository used had no Next.js project files, so the process stopped one step before that). See `CHECKPOINT_1.5.md` for details.

### Q. Does it also auto-fix structured data (JSON-LD) or Open Graph tags?
...
This project has adopted the **Apache License 2.0** (finalized 2026-08-10) — it broadly permits modification, redistribution, and commercial use, provided you preserve copyright/patent notices, mark changed files as changed, and comply with the NOTICE-file terms. That said, the copyright-holder attribution in the `LICENSE` file ("SoDam AI Studio", 2026) and a few other legal-review items (see `HUMAN_ACTION_CHECKLIST.md`) are still pending final expert confirmation. Before any commercial distribution, please check the README's license section and the `LICENSE` file itself, and consult a legal professional if needed. This FAQ is not legal advice.
