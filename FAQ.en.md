...
Once the license is finalized (MIT is intended, pending final copyright holder confirmation), commercial use will be permitted under its terms. Right now, the `LICENSE` file is still a draft — before any commercial distribution, please review section 14 of the README and the `LICENSE` file, and consult a legal professional if needed. This FAQ is not legal advice.
